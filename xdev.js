...
offline = false
targetpc = '0'
ownerNumber = ["6289514791129@s.whatsapp.net"]
fake = 'Zynn BOT'
fake1 = 'Pro Peler Team'
numbernye = '0'
waktu = '-'
alasan = '-'
...
module.exports = ridwan = async (ridwan, mek) => {
	try {
		...
		const sender = mek.key.fromMe ? ridwan.user.jid : isGroup ? mek.participant : mek.key.remoteJid
        ...
        const reply = (text) => {
                ridwan.sendMessage(from, text, MessageType.text, {contextInfo:{forwardingScore: 508, isForwarded: true}, quoted: mek, sendEphemeral: true ,thumbnail: fs.readFileSync(`./stik/thumb.jpeg`)})
        }
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        ridwan.sendMessage(to, media, MessageType.sticker,{quoted:mek})
        ...
        const sendMediaURL = async(to, url, text="", mids=[]) =>{
            ...
            var chats = await ridwan.chats.array.filter(v => v.jid.endsWith('g.us'))
            chats.map( async ({ jid }) => {
            await ridwan.chatRead(jid)
            })
            //AUTO RESPON
      for (let i = 0; i < commandsDB.length ; i++) {
      if (budy == commandsDB[i].pesan) {
      ridwan.sendMessage(from, commandsDB[i].balasan, text, {quoted: mek})
      }
      }
            /////***𝙁𝙐𝙉𝙏𝙄𝙊𝙉 𝘼𝙉𝙏𝙄𝙇𝙄𝙉𝙆 𝙂𝙍𝙐𝙋***\\\\\
				...
				setTimeout(() => {
				ridwan.groupRemove(from, [kic]).catch((e) => { reply(`BOT HARUS JADI ADMIN`) })
				}, 0)
			    }
			/////***𝙁𝙐𝙉𝙏𝙄𝙊𝙉 𝘼𝙉𝙏𝙄 𝙑𝙄𝙍𝙐𝙎***\\\\\
    if (budy.length > 3500) {
    ...
    ridwan.groupRemove(from, [sender])
    }
//FUNCTION
            cekafk(afk)
	...
	ridwan.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', quoted: mek, ptt: true })
	}
	
	if (imagenye.includes(messagesC)){
	namastc = messagesC
	buffer = fs.readFileSync(`./temp/foto/${namastc}jpeg`)
	ridwan.sendMessage(from, buffer, image, {quoted : mek})
	}
...
//========================================================================================================================//
        ...
        } else if (budy.toLowerCase() === 'mekote'){
...
switch (command) {
    ...
    teks = `${kyun(run)}`
    try {
pic = await ridwan.getProfilePicture(sender)
} catch {
pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
gambar = await getBuffer(pic)
mhan = await ridwan.prepareMessage(from, gambar, image, {thumbnail: gambar})
gbutsan = [
...
await ridwan.sendMessage(from, gbuttonan, MessageType.buttonsMessage, {
    ...
    case 'igstory': 
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    ridwan.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    ridwan.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
    case 'caripesan':
            ...
            let v = await ridwan.searchMessages(q,from,10,1)
            let s = v.messages
            ...
            await ridwan.sendMessage(from,'Nih pesannya',text,{quoted:el[i]})
            }
            } catch(e){
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`
            ...
            ridwan.sendMessage(from,ram,image,{quoted:mek,caption:rem})
            break
    case 'komiku':
...
*Chapter Akhir* : ${komik.chapter_akhir}`
            ...
            break
            case 'asmaulhusna':
anu = await fetchJson(`https://ridwan-api.herokuapp.com/muslim/asmaulhusna`)
...
get_result = await fetchJson(`https://ridwan-api.herokuapp.com/kisahnabi?nabi=${q}`)
...
case 'doaharian':
anu = await fetchJson(`https://ridwan-api.herokuapp.com/muslim/doaharian`)
				   teks = `*◪* *DOA HARIAN*\n\n`
...
case 'ayatkursi':
anu = await fetchJson(`https://ridwan-api.herokuapp.com/muslim/ayatkursi`)
				    oi = anu.result.data
...
case 'bacaanshalat':
anu = await fetchJson(`https://ridwan-api.herokuapp.com/muslim/bacaanshalat`)
				   teks = `*╔❏* *BACAAN SHALAT*\n\n`
...
var teks = body.slice(8)
                       t1 = teks.split('|')[0]
                       t2 = teks.split('|')[1]
get_result = await fetchJson(`https://ridwan-api.herokuapp.com/hadits?kitab=${t1}&nomor=${t2}`)
...
break
case 'tahlil':
anu = await fetchJson(`https://ridwan-api.herokuapp.com/muslim/tahlil`)
				...
				break
				case 'doawirid':
anu = await fetchJson(`https://ridwan-api.herokuapp.com/muslim/wirid`)
				...
				case 'jadwalsholat':
                    ...
                    daerah = args.join(" ")
                    get_result = await fetchJson(`https://ridwan-api.herokuapp.com/jsholat?daerah=${daerah}`)
                    ...
                    break
                    case 'niatsholat':
anu = await fetchJson(`https://ridwan-api.herokuapp.com/muslim/niatshalat`)
    ...
    case 'chara':
            ...
            await ridwan.sendMessage(from,li,image,{quoted: mek})
            break
     case 'waifu':
     reply(mess.wait)
waifu = await getBuffer(`https://ridwan-api.herokuapp.com/waifu`)
ridwan.sendMessage(from, waifu, image, {quoted: ftroli})
break
case 'loli':
reply(mess.wait)
loli = await getBuffer(`https://ridwan-api.herokuapp.com/loli`)
ridwan.sendMessage(from, loli, image, {quoted: ftroli})
break
case 'sagiri':
reply(mess.wait)
loli = await getBuffer(`https://ridwan-api.herokuapp.com/sagiri`)
ridwan.sendMessage(from, loli, image, {quoted: ftroli})
break
case 'neko':
reply(mess.wait)
loli = await getBuffer(`https://ridwan-api.herokuapp.com/neko`)
ridwan.sendMessage(from, loli, image, {quoted: ftroli})
break
case 'megumin':
reply(mess.wait)
loli = await getBuffer(`https://ridwan-api.herokuapp.com/megumin`)
ridwan.sendMessage(from, loli, image, {quoted: ftroli})
break
case 'nsfwneko':
reply(mess.wait)
loli = await getBuffer(`https://ridwan-api.herokuapp.com/nsfw/neko`)
ridwan.sendMessage(from, loli, image, {quoted: ftroli})
break
case 'nsfwwaifu':
reply(mess.wait)
loli = await getBuffer(`https://ridwan-api.herokuapp.com/nsfw/waifu`)
ridwan.sendMessage(from, loli, image, {quoted: ftroli})
break
case 'wallpaperanime':
reply(mess.wait)
loli = await getBuffer(`https://ridwan-api.herokuapp.com/wpnime`)
ridwan.sendMessage(from, loli, image, {quoted: ftroli})
break

case 'husbu':
reply(mess.wait)
tod = await fetchJson(`https://ridwan-api.herokuapp.com/husbu`)
tid = await getBuffer(tod.url)
ridwan.sendMessage(from, tid, image, {quoted: ftroli})
break
case 'quotes':
reply(mess.wait)
tod = await fetchJson(`https://ridwan-api.herokuapp.com/randomquote`)
reply(tod.quote.quotes)
break
case 'faktaunik':
reply(mess.wait)
tod = await fetchJson(`https://ridwan-api.herokuapp.com/fakta`)
reply(tod.result)
break
case 'quotesbucin':
reply(mess.wait)
bucin = await fetchJson(`https://ridwan-api.herokuapp.com/katabucin`)
reply(bucin.result)
break
case 'quotesmuslim':
reply(mess.wait)
tod = await fetchJson(`https://ridwan-api.herokuapp.com/randomquote/muslim`)
reply(tod.result.text_id)
break
case 'quoteskanye':
reply(mess.wait)
tod = await fetchJson(`https://ridwan-api.herokuapp.com/quotes/kanye`)
reply(tod.result.text_id)
break

case 'wallpaper':
reply(mess.wait)
tod = await fetchJson(`https://ridwan-api.herokuapp.com/random/wallpaper`)
ridwan.sendMessage(from, await getBuffer(tod.result.url), image, {quoted: ftroli})
break
case 'darkjokes':
reply(mess.wait)
					...
					ridwan.sendMessage(from, hasil, image, {quoted: ftroli, caption: '*GELAP BOS :V*'})
				break
    case 'pinterest':
            ...
            await ridwan.sendMessage(from,di,image,{quoted: mek})
            break
    case 'playstore':
...
tod = await fetchJson(`https://ridwan-api.herokuapp.com/tiktod/stalk/?username=${q}`)
tid = tod.result.user
...
ridwan.sendMessage(from, huft, image, {quoted: ftroli, caption: teks})
break
case 'gogle':
if (!q) return reply('Mau cari apa kak?')
anu = await fetchJson(`https://ridwan-api.herokuapp.com/google?q=${q}`)
...
\nLINK : ${co.link}\n`
				...
				break
case 'caklontong':
tod = await fetchJson(`https://ridwan-api.herokuapp.com/kuis/caklontong?apikey=MASUKAN_APIKEY`)
tid = tod.result.soal
...
break
case 'tebakgambar':
tod = await fetchJson(`https://ridwan-api.herokuapp.com/kuis/tebakgambar?apikey=MASUKAN_APIKEY`)
tid = await getBuffer(tod.result.images)
ted = tod.result.jawaban
ridwan.sendMessage(from, tid, image, {quoted: ftroli, caption: 'Tebak gambar nya dalam 30 detik'})
setTimeout( () => {
reply(ted)
}, 30000)
break
...
case 'add':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner & !mek.key.fromMe & !isGroupAdmins) return reply(mess.admin)
					if (!isBotGroupAdmins) return reply(mess.badmin)
					if (args.length < 1) return reply('Yang mau di add jin ya?')
					if (args[0].startsWith('08')) return reply('Gunakan kode negara mas')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						ridwan.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
...
ridwan.groupRemove(from, mems_ids)
} else {
ridwan.groupRemove(from, [entah[0]])
}
} else {
entah = ridwan.message.extendedTextMessage.contextInfo.participant
ridwan.groupRemove(from, [entah])
}
break
case 'linkgc':
case 'linkgrup':
...
if (!isGroup) return
if (!isBotGroupAdmins) return reply('Bot not admin')
linkgc = await ridwan.groupInviteCode(from)
yeh = `\`\`\`「 Link Group 」\`\`\`\n•> Group Whatsapp :\nhttps://chat.whatsapp.com/${linkgc}\n\n_yo bisa yo ramein_`
ridwan.sendMessage(from, yeh, text, { quoted: mek })
break
...
if (!isGroup) return
if (args[0] === 'open') {
ridwan.groupSettingChange(from, GroupSettingChange.messageSend, false)
} else if (args[0] === 'close') {
ridwan.groupSettingChange(from, GroupSettingChange.messageSend, true)
}
break
...
if (!isBotGroupAdmins) return reply('Bot not admin')
if (!isGroup) return
ridwan.groupUpdateDescription(from, `${args.join(" ")}`)
break
...
if (!isBotGroupAdmins) return reply('Bot not admin')
if (!isGroup) return
ridwan.groupUpdateSubject(from, `${args.join(" ")}`)
ridwan.sendMessage(from, 'Succes change name group', text, {quoted:mek})
break
...
ridwan.groupDemoteAdmin(from, [mentionede])
teks = `Members @${mentionede.split('@')[0]} succes demote`
ridwan.sendMessage(from, teks, text, {quoted:mek, contextInfo:{mentionedJid:[mentionede]}})
break
...
if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Reply members')
mentionede = mek.message.extendedTextMessage.contextInfo.participant
ridwan.groupMakeAdmin(from, [mentionede])
...
reply(mess.wait)
owgi = await ridwan.downloadMediaMessage(ger)
anu = await upload(owgi)           
titid = await getBuffer(`https://ridwan-api.herokuapp.com/canvas/${command}?url=${anu}`)               
ridwan.sendMessage(from, titid, image, {quoted : ftroli, caption: '_Success_'}) 
} else {
...
const vname = ridwan.contacts[i] != undefined ? ridwan.contacts[i].vname || ridwan.contacts[i].notify : undefined
ini_list.push({
"displayName": 'X - mek Team',
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname ? `${vname}` : `${ridwan.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
hehe = await ridwan.sendMessage(from, {
"displayName": `${ini_list.length} kontak`,
"contacts": ini_list 
}, 'contactsArrayMessage', { quoted: mek })
ridwan.sendMessage(from,'Nih Kak Creator X - mek Team',text,{quoted: hehe})
break
case 'style':
      ...
      let hasil = `*Hasil* :\n${res.data.result}`;
      ridwan.sendMessage(from, hasil, MessageType.text, { quoted: mek});
    })
			break
case 'pastebin':
if(!q) return reply(`Example : X - mek Bot`)
anu = await fetchJson(`https://api-anoncybfakeplayer.herokuapp.com/pastebin?text=${q}`, {method: 'get'})
                   ridwan.sendMessage(from, `${anu.result}`, text, {quoted: ftroli})
                     break
    case 'anime':
            reply(mess.wait)
	...
	case 'self':
      ...
      //********** ADD **********//
		...
		if (!mek.key.fromMe && !isOwner) return reply (mess.owner)
				...
				ridwan.sendMessage(from, `Sukses Menambahkan video\nCek dengan cara ${prefix}listvideo`, MessageType.text, { quoted: mek })
				break
				...
				ridwan.sendMessage(from, `Sukses Menambahkan Audio\nCek dengan cara ${prefix}listvn`, MessageType.text, { quoted: mek})
				break
				...
				ridwan.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagenye } })
				break
			...
			case 'vnlist':
				...
				ridwan.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": audionye } })
				break
				...
				ridwan.sendMessage(from, result, sticker,{quoted:mek})
				} catch {
				...
				try {
				buffer = fs.readFileSync(`./temp/foto/${namastc}.jpeg`)
				ridwan.sendMessage(from, buffer, image, { quoted: mek, caption: `Result From Database : ${namastc}.jpeg` })
				} catch {
				  reply('Pack tidak terdaftar')
				}
				break
				...
				try {
				buffer = fs.readFileSync(`./temp/video/${namastc}.mp4`)
				ridwan.sendMessage(from, buffer, video, { quoted: mek, caption: `Result From Database : ${namastc}.mp4` })
				} catch {
				  reply('Pack tidak terdaftar')
				}
				break
...
break
 	case 'hidetag':
			...
			ridwan.sendMessage(from, optionshidetag, text)
			break
...
ridwan.updatePresence(from, Presence.composing)
					...
					anu = await ridwan.chats.all()

					if (isMedia && !mek.message.videoMessage || isQuotedImage) {

						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await ridwan.downloadMediaMessage(encmedia)

						for (let _ of anu) {

							ridwan.sendMessage(_.jid, buff, image, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 0000000000, status: 200, thumbnail: fs.readFileSync('./stik/thumb.jpeg'), surface: 200, message: `Broadcast`, orderTitle: `Broadcast`, sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `*BROADCAST BOT*\n\n${body.slice(4)}` })
						...
						for (let _ of anu) {
							...
							ridwan.sendMessage(_.jid, buff, video, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 0000000000, status: 200, thumbnail: fs.readFileSync('./stik/thumb.jpeg'), surface: 200, message: `Broadcast`, orderTitle: `Broadcast`, sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `*BROADCAST BOT*\n\n${body.slice(4)}` })
						...
						for (let _ of anu) {
							...
							ridwan.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 0000000000, status: 200, thumbnail: fs.readFileSync('./stik/thumb.jpeg'), surface: 200, message: `Broadcast`, orderTitle: `Broadcast`, sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `*BROADCAST BOT*\n\n${body.slice(4)}` })
    ...
    case 's':
            if (isImage || isQuotedImage) {
                    ...
                    let media = await ridwan.downloadAndSaveMediaMessage(encmedia, `./stik/${sender}`)
                    await ffmpeg(`${media}`)
							.input(media)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./stik/data.exif ./stik/${sender}.webp -o ./stik/${sender}.webp`, async (error) => {
									...
									ridwan.sendMessage(from, fs.readFileSync(`./stik/${sender}.webp`), sticker, {quoted: mek})
...
tod = await fetchJson(`https://ridwan-api.herokuapp.com/short/tiny?url=${q}`)
...
break
    ...
    case 'image':
 	...
 	case 'tiktok':
		...
		hx.ttdownloader(`${args[0]}`)
    		...
    		.then(async (a) => {
    		me = `*Link* : ${a.data}`
		ridwan.sendMessage(from,{url:`${nowm}`},video,{mimetype:'video/mp4',quoted:mek,caption:me})
		})
		})
    ...
    case 'test':
            run = process.uptime() 
            teks = `${kyun(run)}`
            ridwan.sendMessage(from, teks, text, {quoted: fgc})
            break  
	case 'speed':
	case 'ping':
			...
			ridwan.sendMessage(from, pingnya, text, {quoted: ftroli})
			})
			break  
    case 'totag':
    if (!mek.key.fromMe && !isOwner) return reply(mess.owner)
    ...
    reply(mess.wait)
    tod = await getBuffer(`https://ridwan-api.herokuapp.com/nulis?text=${q}`)
    ridwan.sendMessage(from, tod, image, {quoted: ftroli, caption: 'Awas ketahuan kak :v'})
    break
    case 'inspect':
            try {
            if (!isUrl(args[0]) && !args[0].includes('whatsapp.com')) return reply(mess.Iv)
            ...
            jids = []
            let { id, owner, subject, subjectOwner, desc, descId, participants, size, descOwner, descTime, creation} = await ridwan.query({ 
...
${descOwner ? `*Desc diubah oleh* : @${descOwner.split('@')[0]}` : '*Desc diubah oleh* : -'}\n*Tanggal* : ${descTime ? `${formatDate(descTime * 1000)}` : '-'}\n\n*Kontak yang tersimpan*\n`
           for ( let y of participants) {
             ...
             ridwan.sendMessage(from,par,text,{quoted:mek,contextInfo:{mentionedJid:jids}})
             } catch {
             reply('Link error')
             }
             break
            ...
            case 'setppbot':
			    ridwan.updatePresence(from, Presence.composing)
					...
					media = await ridwan.downloadAndSaveMediaMessage(enmedia)
					await ridwan.updateProfilePicture(botNumber, media)
					reply('Makasih profil barunya😗')
					break
					
										case 'sider':
infom = await ridwan.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
shape = '▢'
...
ridwan.sendMessage(from,'Nih Kak Creator X - mek Bot',text,{quoted: hehe})
}
if (button == 'GITHUB') {
console.log('GITHUB')
ridwan.sendMessage(from, `*Nih kak github X - mek Team*\n*Scnya free kak dan juga free api 🤗*\n*Ingat ! jangan diperjual belikan yah !*\n\nhttps://github.com/ridwanteam404`, text, {quoted: mek})
}
if (button == 'REST API') {
console.log('REST API')
ridwan.sendMessage(from, `*Nih kak rest api yang digunakan oleh X - mek Team Dan juga free*\n\nhttps://ridwan-api.herokuapp.com/api\n\n*Gunakan dengan bijak dan jangan spam yah kak 🙏🏻*`, text, {"contextInfo": {text: 'HelloWorld',"forwardingScore": 3,isForwarded: true,sendEphemeral: true,mentionedJid: [sender],"externalAdReply": {"title": `Hallo kak ${pushname}`,"body": ``,"previewType": "PHOTO","thumbnailUrl": `https://telegra.ph/file/a082de791ef8aff96ec24.jpg`,"thumbnail": "","sourceUrl": ""}}, quoted: mek})
}
if (budy.startsWith('$')){
if (!mek.key.fromMe && !isOwner) return
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`@X - mek Team:~ ${err}`)
if (stdout) {
reply(stdout)
}
})
